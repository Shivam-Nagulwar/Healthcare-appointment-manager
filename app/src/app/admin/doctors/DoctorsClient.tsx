...
import {
  SearchIcon, PlusIcon, EditIcon,
  TrashIcon, StarFilledIcon, XIcon,
} from '@/components/Icons';
import styles from './page.module.css';
import { saveDoctor, deleteDoctor } from '@/actions/adminDoctors';

function getInitials(name: string) {
  return name.split(' ').filter(n => n).map(n => n[0]).join('').slice(0, 2).toUpperCase();
}

export default function DoctorsClient({ initialDoctors }: { initialDoctors: any[] }) {
  ...
  const filteredDoctors = useMemo(() => {
    if (!search) return initialDoctors;
    const q = search.toLowerCase();
    return initialDoctors.filter(d => 
      d.user.name.toLowerCase().includes(q) || 
      d.specialization.toLowerCase().includes(q)
    );
  }, [search, initialDoctors]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const res = await saveDoctor(formData, editingDoctor?.id);
    setIsSubmitting(false);
                ...
                <tr key={doc.id}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                      <div className="avatar avatar-primary">{getInitials(doc.user.name)}</div>
                      <div>
                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{doc.user.name}</div>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: 'var(--space-2)', justifyContent: 'flex-end' }}>
                      <button className="btn btn-ghost btn-sm btn-icon" onClick={() => handleOpenEdit(doc)} disabled={isSubmitting}>
                        <EditIcon size={16} />
                      </button>
                      <button 
