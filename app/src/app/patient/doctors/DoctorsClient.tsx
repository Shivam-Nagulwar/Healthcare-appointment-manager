...
export default function DoctorsClient({ doctors, user }: { doctors: MappedDoctor[], user: any }) {
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return docs;
  }, [search, selectedSpec, sortBy, doctors]);

  const availableSpecs = ['All', ...new Set(doctors.map(d => d.specialization))];
...
function DoctorCard({ doctor, index, colorIndex }: { doctor: MappedDoctor; index: number; colorIndex: number }) {
  const workingDays = doctor.workingHours ? Object.keys(doctor.workingHours).map(d =>
